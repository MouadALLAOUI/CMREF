...
function RepSyntheseRembClientPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                logger("Failed to load client remboursement synthesis", "error")();
            } finally {
                setIsLoading(false);
            }
        };
    ...
    const clientSummary = useMemo(() => {
        ...
        remboursements.forEach(r => {
            ...
            summary[clientId].totalRemb += r.montant || 0;
            summary[clientId].nbRemb += 1;
        });
        ...
        <div className="space-y-6 max-w-7xl mx-auto p-6">
            <div className="flex items-center justify-between border-b border-slate-100 pb-5">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-black text-slate-900 tracking-tight">Synthèse Remboursement Clients</h1>
                        <p className="text-slate-500 text-xs mt-0.5">Récapitulatif des remboursements par client</p>
                    </div>
                </div>
                    ...
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="bg-slate-50">
                                ...
                                <th className="px-4 py-3 text-center font-bold text-slate-700 border border-slate-200">Nb Remboursements</th>
                                <th className="px-4 py-3 text-right font-bold text-slate-700 border border-slate-200">Total Remboursé (DH)</th>
                            </tr>
                        </thead>
                        <tbody>
                            {clientSummary.map(row => (
                                <tr key={row.clientId} className="border-b border-slate-100 hover:bg-slate-50">
                                    ...
                                    <td className="px-4 py-3 text-center">{row.nbRemb}</td>
                                    <td className="px-4 py-3 text-right font-bold">{row.totalRemb.toLocaleString()} DH</td>
                                </tr>
                            ))}
                            {clientSummary.length > 0 && (
                                <tr className="bg-slate-900 text-white font-bold">
                                    <td className="px-4 py-3">TOTAL</td>
                                    <td className="px-4 py-3 text-center">{clientSummary.reduce((s, r) => s + r.nbRemb, 0)}</td>
                                    <td className="px-4 py-3 text-right">{clientSummary.reduce((s, r) => s + r.totalRemb, 0).toLocaleString()} DH</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
            ...
            <PdfDialogViewer
                ...
                title="Synthèse Remboursement Clients"
                document={
                    <RepSynthesePdf
                        title="Synthèse Remboursement Clients"
                        categoryData={[{ categoryName: "Remboursements", books: clientSummary.map(c => ({ code: c.clientName, livraison: c.totalRemb })) }]}
                        kpis={{ credit: 0, avance: clientSummary.reduce((s, r) => s + r.totalRemb, 0), reste: 0, recouvrement: "0.00" }}
                        repName={user?.name}
                    />
                }
            />
...
export default RepSyntheseRembClientPage;
