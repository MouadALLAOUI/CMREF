...
function RepFactureMSMPage() {
    ...
    const fetchData = async () => {
        ...
        try {
            const [factsRes, livRes, catsRes] = await Promise.all([
                factService.getAll({ annee: activeSeason?.label, type: "MSM-MEDIAS" }),
                livreService.getAll(),
            ...
            logger("Failed to load MSM facture data", "error")();
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            toast.success("Demande de facture envoyée avec succès");
            setFormData({ date: "", ville: "", client: "", ice: "", adresse: "", tel: "", articles: {} });
            fetchData();
        } catch (error) {
            toast.error("Erreur lors de l'envoi de la demande");
            logger("Failed to create demande facture", "error")();
        }
    };
    ...
    return (
        <div className="space-y-6 max-w-7xl mx-auto p-6">
            <div className="flex items-center gap-4 border-b border-slate-100 pb-5">
                ...
                <div>
                    <h1 className="text-2xl font-black text-slate-900 tracking-tight">Facture MSM-MEDIAS</h1>
                    <p className="text-slate-500 text-xs mt-0.5">
                        Liste et demande de factures MSM-MEDIAS
                    </p>
                </div>
            </div>

            <SectionContainer
                title="Liste des factures MSM-MEDIAS"
                icon={FileText}
            ...
            >
                <MyTable
                    data={factures.filter(f => f.type === "MSM-MEDIAS")}
                    columns={columns}
                    pageSize={15}
                    variant="slate"
                    isLoading={isLoading}
...
export default RepFactureMSMPage;
