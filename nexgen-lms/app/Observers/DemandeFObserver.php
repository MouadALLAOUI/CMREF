...
class DemandeFObserver
{
    public function updated(DemandeF $demande)
    {
        if ($demande->wasChanged('livree') && $demande->livree === true) {
            $fact = Fact::where('demande_id', $demande->id)->first();

            if (!$fact) {
                Log::warning("DemandeF {$demande->id} marked as livree but no associated Fact found");
                return;
            }

            DB::transaction(function () use ($fact, $demande) {
                $this->deductStockFromDepot($fact, $demande);
            });
        }
    }

    private function deductStockFromDepot(Fact $fact, DemandeF $demande)
    {
